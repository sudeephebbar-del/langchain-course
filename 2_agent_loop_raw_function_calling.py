...
def ollama_chat_traced(messages):
    client = ollama.Client(host=OLLAMA_HOST, timeout=OLLAMA_TIMEOUT_S)
    return client.chat(
        model=MODEL,
        tools=tools_for_llm,
        messages=messages,
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        print(f"  [LLM] Calling Ollama model='{MODEL}' (timeout={OLLAMA_TIMEOUT_S}s, num_predict={OLLAMA_NUM_PREDICT})")
        ...
        # Keep messages as plain dicts for subsequent Ollama calls.
        messages.append(
            {
                "role": "assistant",
...
if __name__ == "__main__":
    ...
    print()
    result = run_agent("What is the price of a laptop after applying a gold, silver and bronze discounts?")
